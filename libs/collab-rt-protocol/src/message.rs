...
use crate::yrs::updates::decoder::{Decode, Decoder};
use crate::yrs::updates::encoder::{Encode, Encoder};
use crate::yrs::StateVector;
...
impl Decode for Message {
  fn decode<D: Decoder>(decoder: &mut D) -> Result<Self, crate::yrs::encoding::read::Error> {
    ...
    match tag {
      ...
      _ => Err(crate::yrs::encoding::read::Error::UnexpectedValue),
    }
  }
}
...
impl SyncMeta {
  ...
  pub fn from_vec(data: &[u8]) -> Result<Self, crate::yrs::encoding::read::Error> {
    let meta =
      bincode::deserialize(data).map_err(|_| crate::yrs::encoding::read::Error::UnexpectedValue)?;
    Ok(meta)
  }
}
...
impl<D: Decoder> Iterator for MessageReader<'_, D> {
  type Item = Result<Message, crate::yrs::encoding::read::Error>;

  fn next(&mut self) -> Option<Self::Item> {
    match Message::decode(self.0) {
      Ok(msg) => Some(Ok(msg)),
      Err(crate::yrs::encoding::read::Error::EndOfBuffer(_)) => None,
